...
class GitService:
    ...
    def _get_pr_status_for_branch(self, repo_path: Path, branch: str) -> str | None:
    ...
    def analyze_branches_for_github_cleanup(
        self,
        repo_path: Path,
        worktrees: list[WorktreeInfo],
    ) -> list[BranchStatus]:
        ...
        for worktree in worktrees:
            branch = worktree.branch
            pr_status = self._get_pr_status_for_branch(repo_path, branch)
        ...
        logger.debug(f"Analyzed {len(branch_statuses)} branches for GitHub cleanup")
        return branch_statuses
