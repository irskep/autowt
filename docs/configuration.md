...
| `branch_prefix`                | string  | `null`                                | Automatically prefix new branch names with a template. Can use variables `{repo_name}`, `{github_username}` (if `gh` CLI is available), `{gitlab_username}` (if `glab` CLI is available), and environment variables. Examples: `"feature/"`, `"{github_username}/"`, `"{gitlab_username}/"`. When set, `autowt my-feature` creates `feature/my-feature`. Also applies when switching: `autowt my-feature` switches to `feature/my-feature` if it exists. Prevents double-prefixing if the branch name already includes the prefix. <br> **ENV**: `AUTOWT_WORKTREE_BRANCH_PREFIX`   |
