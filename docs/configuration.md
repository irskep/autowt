...
| ------------ | ------- | ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `mode`       | string  | `"tab"` | Determines how `autowt` opens worktrees. <br> • `tab`: Open in a new tab (default). <br> • `window`: Open in a new window. <br> • `inplace`: Switch the current terminal to the worktree directory. <br> • `echo`: Output shell commands to stdout. <br> With [shell integration](terminalsupport.md#shell-integration-alternative-to-terminal-automation) installed, the default becomes `inplace`. <br> **ENV**: `AUTOWT_TERMINAL_MODE` <br> **CLI**: `--terminal <mode>` |
...
| Key                            | Type    | Default                               | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ------------------------------ | ------- | ------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `directory_pattern`            | string  | `"../{repo_name}-worktrees/{branch}"` | The template for creating worktree directory paths. Can use variables `{repo_dir}` (full repo path), `{repo_name}` (repo directory name), `{repo_parent_dir}` (parent directory of repo), `{branch}` (branch name formatted using `flatten_worktree_directories`), and environment variables like `$HOME`. Examples: `"{repo_parent_dir}/worktrees/{branch}"`, `"$HOME/worktrees/{repo_name}/{branch}"`. This can be overridden on a per-command basis using the `--dir` flag. <br> **ENV**: `AUTOWT_WORKTREE_DIRECTORY_PATTERN` <br> **CLI**: `--dir <path>` |
| `flatten_worktree_directories` | boolean | `true`                                | Controls how branch names are expanded for `{branch}` in `directory_pattern`. When `true`, slashes are replaced with hyphens, so `feature/my-thing` becomes `feature-my-thing`. When `false`, slashes are preserved as path separators, so `feature/my-thing` becomes `feature/my-thing`. Explicit `--dir` paths bypass this setting. <br> **ENV**: `AUTOWT_WORKTREE_FLATTEN_WORKTREE_DIRECTORIES`                                                                                                                       |
...
| `branch_prefix`                | string  | `null`                                | Automatically prefix new branch names with a template. Can use variables `{repo_name}`, `{forge_username}` (your username on whichever of GitHub or GitLab hosts the repository), `{github_username}` (if `gh` CLI is available), `{gitlab_username}` (if `glab` CLI is available), and environment variables. Examples: `"feature/"`, `"{forge_username}/"`. When set, `autowt my-feature` creates `feature/my-feature`. Also applies when switching: `autowt my-feature` switches to `feature/my-feature` if it exists. Prevents double-prefixing if the branch name already includes the prefix. <br> **ENV**: `AUTOWT_WORKTREE_BRANCH_PREFIX`   |
...
| Key            | Type   | Default         | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| -------------- | ------ | --------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `default_mode` | string | `"interactive"` | The default mode for the `cleanup` command. <br> • `interactive`: Opens a TUI to let you choose what to remove. <br> • `merged`: Selects branches that have been merged into your main branch. <br> • `remoteless`: Selects local branches that don't have an upstream remote. <br> • `all`: Non-interactively selects all merged and remoteless branches. <br> • `github`: Uses the GitHub CLI (`gh`) to identify branches with merged or closed pull requests. <br> • `gitlab`: Uses the GitLab CLI (`glab`) to identify branches with merged or closed merge requests. <br><br> **First run**: If not configured, autowt will prompt you to select your preferred mode on first use. autowt looks at the `origin` remote and offers `github` or `gitlab` when that service's CLI is installed. <br> **ENV**: `AUTOWT_CLEANUP_DEFAULT_MODE` <br> **CLI**: `--mode <mode>` |
